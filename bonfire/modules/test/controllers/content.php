...
class content extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Test.Content.View');
	...
	*/
	...
	{
		$this->auth->restrict('Test.Content.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_test())
			{
				...
				Template::redirect(SITE_AREA .'/content/test');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/test');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Test.Content.Edit');
		...
		{
			$this->auth->restrict('Test.Content.Delete');
